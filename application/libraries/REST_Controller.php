...
abstract class REST_Controller extends \CI_Controller {
    ...
    public function response($data = NULL, $http_code = NULL)
    {
        ...
        $output = NULL;
        $response_code = isset($data['ResponseCode']) ? $data['ResponseCode'] : "";
        ...
        if ($this->config->item('rest_enable_logging') === TRUE)
        {
            $this->_log_response_code($http_code,$output,$this->response->format,$response_code);
        }
     ...
     * @return bool TRUE log table updated; otherwise, FALSE
     */
    protected function _log_response_code($http_code,$output,$format,$response_code)
    {
        // print_r($response_code);exit();
        $payload['response_code'] = $response_code;
        $payload['response'] = $output;
        $payload['response_format'] = $format;
        $payload['http_response'] = $http_code;

        // if ($http_code == '200') {
        //     $this->rest->db
        //         ->where('api_key', $this->rest->key)
        //         ->set('success', 'success + 1', FALSE)
        //         ->update('applications');
        // }else{
